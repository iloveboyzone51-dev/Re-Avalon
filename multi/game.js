...
function gameLoop(now){
    ...
    ctx.clearRect(0,0,canvas.width,canvas.height);
        ...
        entities.forEach(e => {
            ...
            else if(e.type === 'hero' || e.type === 'minion' || e.type === 'monster' || e.type === 'tower' || e.type === 'nexus' || e.type === 'nexus_turret') drawEntityGeneric(e, ctx);
        });
    ...
    dt=Math.min((now-GS.lastFrame)/1000, 0.2); GS.lastFrame=now;
...
function drawEntityGeneric(e, ctx) {
    ...
    } else if(e.type==='tower' || e.type==='nexus_turret' || e.type==='nexus') { ctx.fillStyle = e.faction==='BLUE' ? '#1e3a8a' : '#7f1d1d'; ctx.fillRect(-e.radius, -e.radius, e.radius*2, e.radius*2); } else if(e.type==='minion') {

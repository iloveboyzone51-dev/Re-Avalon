...
class NetworkManager {
    ...
    broadcastInput(keys, joystick) { if(this.mode !== 'CLIENT' || !this.connections[this.hostId]) return; this.connections[this.hostId].send({ type: 'INPUT', keys: keys, joystick: joystick }); }

    broadcastGameState() {
        if(this.mode !== 'HOST') return;
        // 매우 가벼운 형태로 게임 상태 압축 (Serialization)
        const msg = { type: 'GAME_STATE', state: window.serverGameState }; Object.values(this.connections).forEach(conn => { if(conn.open) conn.send(msg); });
    }
}

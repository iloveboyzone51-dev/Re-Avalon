...
"""
    ...
    # 3. Replace handleAI (stops strictly before onKill)
    handle_ai_pattern = re.compile(r'    handleAI\(dt\).*?(?=    autoAttack\(\)\{)', re.DOTALL)
...
"""
    ...
    # 4. Replace aiShopAI (stops strictly before buyItem)
    shop_ai_pattern = re.compile(r'    aiShopAI\(\)\s*\{.*?(?=    buyItem\()', re.DOTALL)
...
"""
    ...
    print("AI Evolution Update applied 100% SAFELY!")

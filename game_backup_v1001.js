// ======================================================
// 운빨 아발론 - 5:5 AOS 게임 엔진 v3.1 (피드백 반영판)
// 부드러운 사운드, BGM 수정, 밸런스 패치, 스킬 레벨업 등
// ======================================================
...
const HERO_TMPL = {
    BERSERKER: { name:"광전사", color:"#ef4444", hp:1800, atk:60, aspd:1.5, move:190, range:90, type:"melee", skill1:{name:"회전 참격", cd:5}, skill2:{name:"도약 강타", cd:8}, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'berserker') },
    ARCHER: { name:"궁수", color:"#10b981", hp:700, atk:35, aspd:1.2, move:150, range:450, type:"ranged", skill1:{name:"화살 폭우", cd:6}, skill2:{name:"백스텝", cd:10}, critChance:0.1, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'archer') },
    NECROMANCER: { name:"네크로맨서", color:"#a855f7", hp:800, atk:25, aspd:1.0, move:140, range:360, type:"ranged", skill1:{name:"해골 소환", cd:8}, skill2:{name:"저주 역병", cd:12}, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'necromancer') },
    MECHANIC: { name:"메카닉", color:"#f59e0b", hp:900, atk:40, aspd:0.8, move:130, range:300, type:"ranged", skill1:{name:"터렛 설치", cd:12}, skill2:{name:"긴급 수리", cd:15}, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'mechanic') },
    VAMPIRE: { name:"뱀파이어", color:"#f43f5e", hp:1600, atk:45, aspd:1.3, move:180, range:100, type:"melee", skill1:{name:"흡혈 파동", cd:8}, skill2:{name:"박쥐 강습", cd:10}, lifeSteal:0.25, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'vampire') },
    THOR: { name:"토르", color:"#60a5fa", hp:2000, atk:70, aspd:0.9, move:180, range:90, type:"melee", skill1:{name:"번개 강타", cd:10}, skill2:{name:"충격파", cd:12}, draw:(ctx,x,y,r,dir,f)=>drawBlockyHero(ctx,x,y,r,dir,f,'thor') }
};
...
// ============ 전역 상태 ============
let GS = { status:'TITLE', platform:'PC', faction:'BLUE', hero:'BERSERKER', time:0, lastFrame:0 };
...
class Entity {
    ...
    update(dt){
        ...
        this.x=clamp(this.x+this.vx*dt, 10, MAP_SIZE-10);
        this.y=clamp(this.y+this.vy*dt, 10, MAP_SIZE-10);
        this.attackTimer-=dt;
    ...
    applyRawDamage(amount, attacker, triggerEffects=true){
        ...
        addText(this.x+rand(-15,15), this.y-this.radius-10, isCrit?`💥${dmg}`:dmg, attacker===player?'#fbbf24':'#f8fafc', isCrit?18:14);

        if(this.hp<=0){ this.hp=0; this.isDead=true; if(attacker?.onKill) attacker.onKill(this); this.onDeath(attacker); }
        playSFX('hit');
        return dmg;
    }
    onDeath(attacker){}
...
class Hero extends Entity {
    constructor(x, y, faction, heroKey, isPlayer=false, laneRole='mid'){
        ...
        this.inventory=[];
        this.respawnTimer=0; this.skill1Timer=0; this.skill2Timer=0;
        this.skillLevel=1;
    ...
    update(dt){
        ...
        this.skill1Timer-=dt; this.skill2Timer-=dt;
    ...
    handleAI(dt){
        ...
        if(true) {
            ...
            if(this.target){
                ...
                if(this.skill1Timer<=0&&d<=500) this.useSkill(1);
                else if(this.skill2Timer<=0&&d<=400) this.useSkill(2);
            } 
            // 적이 없을 때: 라인으로 이동
            else {
    ...
    useSkill(idx){
        let k=this.heroKey; let cd=(idx===1?HERO_TMPL[k].skill1.cd:HERO_TMPL[k].skill2.cd) - (this.skillLevel*0.5); // 레벨업 시 쿨타임 감소
        cd = Math.max(2, cd);
        if(idx===1){ if(this.skill1Timer>0) return; this.skill1Timer=cd; } else { if(this.skill2Timer>0) return; this.skill2Timer=cd; }
        let nearEnemies=(cx,cy,r)=>entities.filter(e=>e.faction!==this.faction&&!e.isDead&&dist({x:cx,y:cy},e)<=r);
        playSFX('shoot');
        
        let skillDmg = this.atk * (1.5 + this.skillLevel * 0.5); // 스킬 데미지도 스킬 레벨에 비례

        // 스킬 범위 시각화 (인디케이터)
        if(k==='BERSERKER' || k==='THOR'){
            let hits=nearEnemies(this.x,this.y,250);
            hits.forEach(e=>{e.applyRawDamage(skillDmg,this);e.stunTimer=1;});
            spawnAOE(this.x,this.y,250,HERO_TMPL[k].color+'88',0.5); addText(this.x,this.y-50,HERO_TMPL[k][`skill${idx}`].name, HERO_TMPL[k].color);
        } else {
            let t=this.target||entities.filter(e=>e.faction!==this.faction&&!e.isDead).sort((a,b)=>dist(this,a)-dist(this,b))[0];
            if(t){
                if(HERO_TMPL[k].type==='ranged') {
                    for(let i=0;i<3+this.skillLevel;i++) setTimeout(()=>projectiles.push(new Projectile(this.x,this.y,t,skillDmg*0.5,this,false)), i*100);
                } else {
                    this.x=t.x+rand(-40,40); this.y=t.y+rand(-40,40); t.applyRawDamage(skillDmg*1.5,this);
                    spawnParticles(this.x,this.y,HERO_TMPL[k].color,20,150,0.5);
                }
                addText(this.x,this.y-50,HERO_TMPL[k][`skill${idx}`].name, HERO_TMPL[k].color);
            }
        }
    }
    onKill(target){
    ...
    gainExp(amt){
        ...
        while(this.exp>=this.maxExp){
            this.exp-=this.maxExp; this.level++; this.maxExp=Math.floor(this.maxExp*1.2);
            this.skillLevel = Math.floor((this.level - 1) / 3) + 1; // 3레벨마다 스킬 레벨업
...
class Minion extends Entity {
    constructor(x,y,faction,lane){
        ...
        let scale=1+GS.time/300; this.maxHp=Math.floor(300*scale); this.hp=this.maxHp; this.atk=Math.floor(15*scale); this.aspd=1.0; this.moveSpd=120; this.range=30; this.radius=10;
...
class Monster extends Entity {
    constructor(x,y,mtype){
        ...
        this.maxHp = mtype.includes('boss') ? 5000 : 1200; this.hp=this.maxHp;
...
// ============ 입력 이벤트 & 줌 ============
window.addEventListener('keydown',e=>{ let k=e.key.toLowerCase(); if(keys.hasOwnProperty(k)) keys[k]=true; if(k==='o'&&player) player.useSkill(1); if(k==='p'&&player) player.useSkill(2); });
...
window.selectHero=h=>{ GS.hero=h; Object.keys(HERO_TMPL).forEach(hk=>{ document.getElementById('btnHero'+hk).className=`py-2 px-1 rounded-xl border-2 ${hk===h?'border-emerald-500 bg-slate-800/80':'border-transparent bg-slate-800/60'} flex flex-col items-center transition-all`; }); document.getElementById('heroDescription').innerHTML=`<div class="text-amber-400 font-bold mb-0.5">[${HERO_TMPL[h].name}] <span class="text-[10px] text-emerald-400 font-bold ml-1">상세 스킬 정보</span></div><div class="text-white bg-slate-900 p-1 rounded mt-1 text-[10px]">✨스킬1: <span class="text-emerald-300 font-bold">${HERO_TMPL[h].skill1.name}</span> (쿨타임 ${HERO_TMPL[h].skill1.cd}초) <br/> 💫스킬2: <span class="text-indigo-300 font-bold">${HERO_TMPL[h].skill2.name}</span> (쿨타임 ${HERO_TMPL[h].skill2.cd}초)</div><div class="text-[9px] text-slate-400 mt-1">※ 3레벨 오를 때마다 스킬 데미지가 증가하고 쿨타임이 감소합니다.</div>`; };
...
window.startGame=()=>{
    ...
    GS.scoreBlue=0; GS.scoreRed=0; GS.time=0; minionTimer=MINION_INTERVAL-2;
    ...
    document.getElementById('txtSkill1Icon').textContent = GS.hero==='ARCHER'?'🏹':(GS.hero==='NECROMANCER'?'💀':(GS.hero==='MECHANIC'?'🔧':(GS.hero==='VAMPIRE'?'🦇':(GS.hero==='THOR'?'⚡':'✨'))));
    document.getElementById('txtSkill2Icon').textContent = '💫';
...
function gameLoop(now){
    ...
    let dt=Math.min((now-GS.lastFrame)/1000, 0.2); GS.lastFrame=now; GS.time+=dt;
    ...
    for(let i=0; i<3; i++) {
        if(!midBossSpawned[i] && GS.time >= MID_BOSS_TIMES[i]) {
            ...
            showBanner(`시간 경과! 거대 몬스터 출현!`, '🐉', true);
        }
    }
...
function updateUI(){
    ...
    for(let i=0;i<8;i++){ let item=player.inventory[i]; let content=item?`<span class="text-sm">${BASE_ITEMS.find(b=>b.id===item.id)?.icon||'?'}</span>${item.upgrade>0?`<span class="absolute -top-1 -right-1 text-[7px] bg-rose-600 text-white rounded px-0.5 font-bold">+${item.upgrade}</span>`:''}`:''; inv.innerHTML+=`<div class="relative w-6 h-6 md:w-8 md:h-8 rounded bg-slate-900 border border-slate-800 flex items-center justify-center">${content}</div>`; }
    ['1','2'].forEach(n=>{ let mask=document.getElementById('maskSkill'+n); let cd=n==='1'?player.skill1Timer:player.skill2Timer; if(mask) { if(cd>0){mask.classList.remove('hidden');mask.textContent=cd.toFixed(1);}else mask.classList.add('hidden'); } });
}
